  ...
  ### 3. K-POP 스케줄 데이터 수집 및 가공 파이프라인                                                 
   
  Selenium으로 K-POP 아티스트 스케줄 데이터를 수집하고, 서비스에서 활용할 수 있도록 가공합니다.      
                                                                                                   
  - Selenium Headless 브라우저로 blip.kr의 Nuxt.js SSR 데이터(`window.__NUXT__`) 직접 추출           
  - 아티스트별 월간 스케줄 순회 수집 (방송/발매/행사/축하 타입 필터링)                             
  - 동일 스케줄 기간 병합 (아티스트 그룹핑 → 스케줄 타이틀 그룹핑 → start/end 통합)                  
  - 중복 제거 후 PostgreSQL에 artist upsert + schedule 일괄 적재                                     
  - 멤버 정보(한/영), 콘텐츠(이미지/영상), 채널/장소 메타데이터 구조화
